...
int main()
{
	...
	//text settings(main menu) {sameh}
	bool hover2 = false, hover3 = false;
	Font font;
	font.loadFromFile("font.TTF");
	Text text1, text2, text3, text4;
	text1.setFont(font); text2.setFont(font); text3.setFont(font); text4.setFont(font);
	text1.setString("BUBBLE SHOOTER"); text2.setString("START"); text3.setString("EXIT"); text4.setString("LOADING...");
	text1.setCharacterSize(100); text2.setCharacterSize(70); text3.setCharacterSize(60); text4.setCharacterSize(70);
	text1.setPosition(500, 120); text2.setPosition(860, 540); text3.setPosition(900, 780); text4.setPosition(900, 780);
	...
	buffer2.loadFromFile("sound2.ogg");									  
	Sound sound;														  
	Sound music;														  
	...
	//random generation for balls {bahaa}
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 38; j++)
		{
	...
	CircleShape target;
	bool isShot = false, start = false, exit = false, lock = false;
	float speedx, speedy, positionx = 960, positiony = 980;
	int x, y;
	double angle;
	target.setRadius(25);
	target.setPosition(960, 980);
	//random generation for target(single ball)  {bahaa}
	for (int i = 0; i < 1; i++)
	{
			...
			if (isShot)
			{
				...
				for (int i = 0; i < rowmax; i++)
				{
					...
					for (int j = 0; j < columnmax; j++)
					{
						...
						{
							cout << "hit " << (target.getFillColor() == p.color ? "similar" : "cross") << " target\n";
